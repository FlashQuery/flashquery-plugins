...
**Tools:** `write_document` → `insert_doc_link`

Use when the user says something like "start a new doc about X and link it to Y."

1. Call `write_document(mode: "create")` (as above). Parse the returned `fq_id`.
2. Call `insert_doc_link`:
   - `identifiers` — the new document (use the `fq_id` or returned path)
   - `target_identifier` — the document to link to (use whatever identifier the user provided)
   - `property` — default `"links"`; override if the user specifies a relationship type like "parent" or "related"
3. Optionally call `write_memory(mode: "create")` to record the relationship if it's likely to be useful across sessions.

---

## Create-then-save-memory

**Tools:** `write_document` → `write_memory`

Use when writing up notes, summaries, or anything that surfaces a key takeaway worth retaining.

1. Call `write_document(mode: "create")` (as above).
...
3. Call `write_memory(mode: "create")` for each:
...
→ `write_document(mode: "create", title: "Acme Call Notes - [date]", tags: ["#type/meeting-notes", "#client/acme"])` → `write_memory(mode: "create")` (key takeaways)

**"Start a new research doc on vault sync and connect it to the FQC roadmap"**
→ `write_document(mode: "create", title: "Vault Sync Research", tags: ["#type/research", "#status/active"])` → `insert_doc_link` (target_identifier: "FQC roadmap")

**"Create a proposal draft for Acme's AI integration project"**
→ `write_document(mode: "create", title: "Acme AI Integration Proposal", path: "clients/acme/proposal.md", tags: ["#type/proposal", "#client/acme", "#status/draft"])`

**"Start a proposal for Acme and mark it high priority with a due date"** — use `frontmatter` to set custom fields up front:
```
write_document(
  mode: "create",
  title: "Acme Proposal",
