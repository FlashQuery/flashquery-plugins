...
This skill orchestrates FlashQuery's document and memory write tools. Its job is to figure out what the user wants to capture or organize, choose the right tool sequence, and execute it cleanly.
...
- Generic plugin record writes when the user supplies the plugin/table schema context

Tool surface includes `write_document`, `insert_in_doc`, `replace_doc_section`, `apply_tags`, `insert_doc_link`, `archive_document`, `remove_document`, `write_memory`, `archive_memory`, `write_record`, `get_record`, `archive_record`, plus `get_document` as the read-side helper for body, frontmatter, headings, and section extraction. When LLM is configured: `call_model` for AI-assisted content generation. Use `call_macro` only for explicit macro execution requests.
...
| Create, update, or archive a plugin record with known plugin/table context | → [Record Management](workflows/record-management.md) |
| Run an explicit FlashQuery macro | → [Macro Execution](workflows/macro-execution.md) |
...
- **File not tracked (no `fq_id`)** — call `get_document` first to auto-provision FlashQuery metadata, then retry the mutation.
- **Tag validation failure** — FlashQuery rejects duplicate tags after trimming/lowercasing. Normalize the tag list, remove duplicates, then retry.
- **File already exists** — use `write_document(mode: "update")`, `insert_in_doc`, or `replace_doc_section` depending on whether the user wants a full-body or targeted edit.
...
- Always prefer `fq_id` over vault paths for any document reference you need to store or use later — paths can change when users rename or move files.
- After `write_document`, parse the `fq_id` from the JSON response.
