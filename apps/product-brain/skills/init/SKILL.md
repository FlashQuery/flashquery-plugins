...
description: >
  ...
  Product Brain infrastructure in FlashQuery.
---
...
**2. First project name** — the name of their first product or initiative (e.g., "FlashQuery", "My App", "Client Project"). Every document in the Product Brain belongs to a project, so at least one must exist before anything can be captured.
...
Read the schema file at `references/schema.yaml` (in the plugin root — navigate two directory levels up from this SKILL.md). Read the full file content as a string.

Before registering, update the `_plugin` folder entries to prepend the vault root, and append four folder entries for every project declared during setup. For each project (e.g., path `flashquery/` under vault root `product-brain/`), the full modified `documents.types` section should look like:
...
- `content`: a structured configuration note. Include the vault root and all projects. Example for two projects:
  ...
  ```
  [product-brain-config] Vault root: product-brain/. Active projects: FlashQuery (path: product-brain/flashquery/), Client Portal (path: product-brain/client-portal/). Plugin ID: product-brain. Templates in: product-brain/_plugin/templates/. Tags file: product-brain/_plugin/tags.md.
  ```
...
- The schema's `documents.types` section declares folder ownership claims for FlashQuery's plugin manifest system. Project folder entries are appended here during Init so FlashQuery knows which folders belong to this plugin.
