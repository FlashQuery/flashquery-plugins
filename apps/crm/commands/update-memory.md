...
Update an existing CRM memory in FlashQuery when information has changed. This searches for the matching memory, confirms with the user, then creates a new version via `write_memory`. The old version is preserved in the version chain.
...
2. Search for the existing memory — call `mcp__flashquery__search` with:
...
4. Once the user confirms, call `mcp__flashquery__write_memory` with:
   - `mode`: `"update"`
...
- Do NOT call `write_memory` without showing the user what will be replaced first.
- `write_memory` update mode does not accept `plugin_scope` — the scope is inherited from the original.
...
Action: search for `"Sarah communication preferences"` with tags `["crm"]`, show match, confirm, then write_memory with `mode: "update"`, the confirmed `memory_id`, and new content `[communication_preferences] Sarah Chen now prefers Slack over email for routine communication.`
