...
Search CRM memories in FlashQuery. This bypasses Claude's built-in memory and queries FlashQuery directly via `search`, filtered to CRM-scoped memories only.

## What to do

1. Parse $ARGUMENTS to determine the search intent — a contact name, company name, deal topic, or general CRM query.

2. Call `mcp__flashquery__search` with:
...
Action: search with query `"Sarah Chen"`, tags `["crm"]`

User: `/crm:recall communication preferences for Acme contacts`
Action: search with query `"Acme communication preferences"`, tags `["crm"]`

User: `/crm:recall any deal signals from this quarter`
Action: search with query `"deal context pricing budget timeline"`, tags `["crm"]`
